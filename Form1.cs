...
namespace Demodulador_WinForm_1
{
    public partial class Demodulador_DSC : Form
    {
        ...
        private void combox_dispositivos_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (_isCapturing)
            {
                DISPLAYSECUNDARIO.AppendText("Cambiando dispositivo de captura...\n");
                _capturaDatos.CambiarDispositivo();
            }
            else if (combox_hf_vhf.SelectedIndex >= 0)
            {
        ...
        public void RegistrarMensajeRecibido(bool correcto)
        {
            if (InvokeRequired)
            {
                Invoke(() => RegistrarMensajeRecibido(correcto));
                return;
            }

            _mensajesRecibidosTotales++;
            if (correcto)
        ...
        {
            label_recibidos_totales_valor.Text = _mensajesRecibidosTotales.ToString();
            label_demodulados_correctos_valor.Text = _mensajesDemoduladosCorrectos.ToString();
        }

        private void button_reiniciar_contador_Click(object sender, EventArgs e)
        {
            _mensajesRecibidosTotales = 0;
            _mensajesDemoduladosCorrectos = 0;
            ActualizarContadoresMensajes();
        }

        public double ObtenerFiltroPasabajosRxHz()
        {
            if (InvokeRequired)
                return (double)Invoke(new Func<double>(ObtenerFiltroPasabajosRxHz));

            return (double)num_filtro_rx.Value;
        }

        public bool UsarDemodulacionCorrelacion()
        {
            if (InvokeRequired)
                return (bool)Invoke(new Func<bool>(UsarDemodulacionCorrelacion));

            return combox_metodo_demod.SelectedIndex == 1;
        }

        private void combox_metodo_demod_SelectedIndexChanged(object sender, EventArgs e)
        {
            string metodo = combox_metodo_demod.SelectedItem?.ToString() ?? "Actual";
            DISPLAYSECUNDARIO.AppendText($"Método de demodulación seleccionado: {metodo}\n");

            if (_isCapturing)
            {
                DISPLAYSECUNDARIO.AppendText("Cambiando método de demodulación...\n");
                _capturaDatos.CambiarModo();
            }
        }
